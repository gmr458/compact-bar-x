...
fn tab_line_prefix(session_name: Option<&str>, mode: InputMode, palette: Palette) -> Vec<LinePart> {
    let bg_mode = match palette.theme_hue {
    ...
    let mode_part_styled_text = if mode == InputMode::Locked {
        style!(locked_mode_color, bg_mode)
            .bold()
            .paint(mode_part_padded)
    } else if mode == InputMode::Normal {
        style!(normal_mode_color, bg_mode)
            .bold()
            .paint(mode_part_padded)
    } else {
        style!(other_modes_color, bg_mode)
            .bold()
            .paint(mode_part_padded)
    };
    parts.push(LinePart {
